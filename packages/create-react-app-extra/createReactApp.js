...
const program = new commander.Command(packageJson.name)
  ...
  .option(
    ...
    'use a non-standard version of react-scripts-extra'
  )
  .allowUnknownOption()
  .on('--help', () => {
    ...
    console.log(
      `      - a custom fork published on npm: ${chalk.green('my-react-scripts-extra')}`
    );
    console.log(
      `      - a .tgz archive: ${chalk.green('https://mysite.com/my-react-scripts-extra-0.8.2.tgz')}`
    );
    console.log(
    ...
    console.log(
      `      ${chalk.cyan('https://github.com/viankakrisna/create-react-app-extra/issues/new')}`
    );
    console.log();
  })
  .parse(process.argv);
...
if (typeof projectName === 'undefined') {
  ...
  console.log(
    `  ${chalk.cyan(program.name())} ${chalk.green('my-react-app-extra')}`
  );
  console.log();
  console.log(
...
function run(root, appName, version, verbose, originalDirectory, template) {
  ...
  getPackageName(packageToInstall)
    ...
    .then(packageName => {
      ...
      // Since react-scripts-extra has been installed with --save
...
function getInstallPackage(version) {
  let packageToInstall = 'react-scripts-extra';
...
function getPackageName(installPackage) {
  if (installPackage.indexOf('.tgz') > -1) {
    return getTemporaryDirectory()
      ...
      .catch(err => {
        // The package name could be with or without semver version, e.g. react-scripts-extra-0.2.0-alpha.1.tgz
    ...
    // git+https://github.com/mycompany/react-scripts-extra.git
    // git+ssh://github.com/mycompany/react-scripts-extra.git#v1.2.3
...
function checkAppName(appName) {
  ...
  const devDependencies = ['react-scripts-extra'];
...
// https://github.com/viankakrisna/create-react-app-extra/pull/368#issuecomment-243446094
